...
@pytest.mark.skip
class TestC15449(BaseTest):

    """
    Check context menu items on text message in p2p chat
    """
    FRIEND_NAME = config.AMERICA_FIRSTNAME + ' ' + config.AMERICA_LASTNAME
    TEXT_MESSAGE = magic.get_text_message

    def test_c15449(self):
        log.info("Check context menu items on text message in p2p chat")
