...
class TestC11217(BaseTest):
    """
    User has the ability to send voice message in p2p chat with airplane mode
    """
    FRIEND = config.UKRAINE_FIRSTNAME + ' ' + config.UKRAINE_LASTNAME
    MESSAGE_TYPE = 'voice'
    SEND_STATUS = 'Delivered'

    def test_c11217(self):
        log.info("Send voice message in p2p chat with airplane mode")
